...
try:
    ...
    from .beam_checker import BEAM_LABEL_RE, detect_label_units, label_dim_to_mm
except ImportError:
    from parser import strip_mtext_codes, get_line_points, distance
    from beam_checker import BEAM_LABEL_RE, detect_label_units, label_dim_to_mm
...
class BeamSectionParser:
    ...
    def find_beam_labels(self):
        ...
        for e in self.msp:
            ...
            name = match.group(1)
            b = label_dim_to_mm(int(match.group(2)), unit_sys)
            h = label_dim_to_mm(int(match.group(3)), unit_sys)
            x, y, _ = e.dxf.insert
